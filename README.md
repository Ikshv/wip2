...
- **Piper UI:** `http://<host>:8080` — dashboard: **Piper** voices (same volume as TTS), **OpenWakeWord** custom `.onnx` models (same volume as `mara-wakeword`), and a **live tail** of `mara-wakeword` logs via Server-Sent Events (`WAKE_LOG_FILE` on shared `mara_logs` volume). Restart the wake container after uploading or removing wake models.
